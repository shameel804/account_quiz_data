...
Usage:
    ...
    python3 generate_pipeline.py prompts "Accounting"
    python3 generate_pipeline.py prompts --all

    # Generate prompts for a single topic
    python3 generate_pipeline.py prompts "Accounting" --topic basic

    # Process staged AI outputs → validate + merge + output
    python3 generate_pipeline.py process "Accounting"
    python3 generate_pipeline.py process "Accounting" --topic basic
    python3 generate_pipeline.py process --all

    # Validate an existing JSON file
    python3 generate_pipeline.py validate v3/Accounting/basic.json

    # Show status of all topics
    python3 generate_pipeline.py status
    python3 generate_pipeline.py status "Accounting"
"""
import argparse
import json
import sys
...
Examples:
  ...
  python3 generate_pipeline.py status
    ...
    prompts_parser.add_argument("category", nargs="?", help="Category name (e.g., 'Accounting')")
